...
type SysUserAddDto struct {
	...
	Gender   constants.UserGender `json:"gender"`    //用户性别（0未知，1男，2女）
	Status   constants.UserStatus `json:"status"`    //帐号状态（0正常 1停用）
...
type SysUserVo struct {
	...
	UserName  string               `json:"userName"`  //用户账号
	NickName  string               `json:"nickName" ` //用户昵称
	UserType  constants.UserType   `json:"userType"`  //用户类型（0普通账号，1超级管理员）
	Email     *string              `json:"email" `    //用户邮箱
	Phone     *string              `json:"phone"`     //手机号码
	Gender    constants.UserGender `json:"gender"`    //用户性别（0未知，1男，2女）
